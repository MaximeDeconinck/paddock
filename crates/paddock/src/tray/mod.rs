...
mod macos {
    ...
    /// P glyph approximating the Onoma wordmark (geometric sans, rounded
    /// caps: vertical stem + rounded bowl on the upper half); proper
    /// rendered asset later.
...
pub fn run() -> anyhow::Result<()> {
    anyhow::bail!("paddock tray is macOS-only for now")
}
