...
/// Merge tetro-launched servers (registry) with live Ollama state (`/api/ps`)
...
pub fn build_menu_model(
    ...
    // Registry records for an ollama tetro booted are skipped below: the
    ...
    // One section per tetro-launched server (llama-server / mlx_lm.server).
    for r in records {
    ...
    if !runtimes.ollama.installed && !runtimes.llama_cpp.installed && !runtimes.mlx.installed {
        entries.push(MenuEntry::Info(
            "Aucun runtime installé — lance `tetro run` pour installer".into(),
        ));
    }
...
mod tests {
    ...
    fn ollama_registry_record_deduped() {
        // tetro booted `ollama serve` → registry record; the daemon section
    ...
    fn empty_all() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                MenuEntry::Info("Aucun runtime installé — lance `tetro run` pour installer".into()),
            ]
        );
    }
